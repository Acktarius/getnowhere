...
- Operator runbook added to `poke-gateway/README.md` (§ ntfy server maintenance): `ntfy access everyone 'gnh-*' read-only`, delete the `gnh-reader` user, keep `auth-default-access: deny-all`, plus cache/log/rate-limit hardening. **Wake stays broken until step 1 runs** (SSE returns `403`). Corrected 2026-09-24 after operator feedback: ntfy is a container on the VPS, so every command is `sudo docker exec ntfy ntfy …` — the first draft wrongly assumed a host-installed CLI. Runbook now also covers `/v1/health` plus anonymous-read verification before deleting `gnh-reader`, and an auth-file persistence check before any container recreate (recreating without a mounted `auth-file` would drop `gnh-publisher` and break iOS wake too).
