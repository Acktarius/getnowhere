...
child can stay up under init with Hyperswarm still live. Packaged (and any owned
sidecar spawn) enables parent-death watching: on Linux prefer
`PR_SET_PDEATHSIG(SIGTERM)` when available via `process.binding` / native
fallback is optional; portable path is a short-interval `process.ppid` check
that exits when the parent PID changes or is 1. Document that graceful shutdown
